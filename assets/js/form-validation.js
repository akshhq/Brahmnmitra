...
/* ============================================================
   ...
   here is re-checked in enquiry.php, which is the only thing that
   ...
   Behaviour:
     ...
     · Submit  → POST to enquiry.php, JSON response
...
BM.initForm = function () {
  ...
  function validateAll() {
    ...
    if (first) { first.focus(); first.scrollIntoView({ behavior: BM_MOTION(), block: 'center' }); }
    return ok;
  }
  function BM_MOTION() {
  ...
  function summary() {
    ...
    const svcLabel = (svc && svc.value) ? svc.textContent.trim() : 'Travel enquiry';
    ...
    if (isFlight()) {
      const trip = { round_trip: 'Round trip', one_way: 'One way', multi_city: 'Multi-city' }[d.trip_type] || 'Trip';
      L.push(trip + ' · ' + (d.from_city || '?') + ' → ' + (d.to_city || '?'));
      L.push('Depart: ' + (d.depart_date || 'flexible') +
             (d.return_date && d.trip_type !== 'one_way' ? ' · Return: ' + d.return_date : ''));
      L.push((d.passengers || 1) + ' passenger(s), ' + (d.cabin_class || 'economy'));
    }
  ...
  if (waBtn && WA) {
    waBtn.addEventListener('click', () => {
      window.open('https://wa.me/' + WA + '?text=' + encodeURIComponent(summary()),
                  '_blank', 'noopener');
    });
  }
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      ...
      toast.innerHTML = 'Could not send just now — tap ' +
